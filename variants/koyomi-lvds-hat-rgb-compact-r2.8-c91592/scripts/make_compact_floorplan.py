#!/usr/bin/env python3
"""Create the r2.2 compact floorplan with backlight FFC via KiCad IPC.
...
def board_label(
    ...
    item.value = value
    item.attributes.size = Vector2.from_xy_mm(size_mm, size_mm)
...
def main() -> None:
    ...
    with KiCad(
        ...
        reimport = [
            fp
            for fp in footprints
            if fp.reference_field.text.value
            in ({f"RN{i}" for i in range(1, 7)} | {"JBL1"})
        ]
        board.remove_items(reimport)
        ...
        if result.error_count or result.new_footprint_count != 7:
            raise RuntimeError(
                "RN/JBL1 re-import failed: "
        ...
        for fp in footprints:
            ...
            if ref == "JBL1":
                # Use one deliberate board-level label below instead of the
                # footprint reference directly above the latch.  Keeping the
                # imported reference visible made it collide with the revision
                # text in the first r2.2 render.
                fp.reference_field.visible = False
                fp.value_field.visible = False
            updates.append(fp)
        ...
        board.create_items(
            [
                ...
                board_label("JBL1 BACKLIGHT", (141.5, 39.0), 0.7),
                board_label("R2.2", (152.0, 39.0), 0.7),
            ]
        )
        board.save()
