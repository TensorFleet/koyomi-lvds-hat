#!/usr/bin/env python3
"""Build and fail-closed audit the compact LCD R2.6 JLCPCB candidate."""
...
SOURCE = ROOT / "variants/koyomi-lvds-hat-rgb-compact-r2.6-production"
...
DESTINATION = ROOT / "fab/r2.6-compact-rgb-production-jlc-candidate"
PREFIX = "koyomi-lvds-rgb-compact-r2.6-jlc"
EXPECTED_REFS = {
...
def load_positions(path):
    ...
    if {row["Side"] for row in rows} != {"top"}:
        raise RuntimeError("R2.6 must remain single-side assembly")
    return rows
...
def main():
    with tempfile.TemporaryDirectory(prefix=".lcd-r26-jlc-", dir=ROOT) as temp:
        work = Path(temp)
        ...
        if "[excluded]" in errors_drc.read_text(encoding="utf-8"):
            raise RuntimeError("R2.6 contains an error-level DRC exclusion")
        warning_types = Counter(re.findall(
            r"^\[([^]]+)\]", all_drc.read_text(encoding="utf-8"), re.MULTILINE
        ))
        if warning_types != EXPECTED_DRC:
            raise RuntimeError(f"R2.6 all-severity baseline changed: {warning_types}")
        ...
        if '"result": "PASS"' not in audit:
            raise RuntimeError("R2.6 routed-interface audit failed")
        ...
        run(KICAD_CLI, "pcb", "export", "step", "--force", "--no-dnp",
            "--subst-models", "-o", mechanical / f"{PREFIX}.step", BOARD)
        ...
        (DESTINATION / "README.md").write_text(
            "# Compact RGB LCD R2.6 JLCPCB fabrication candidate\n\n"
        ...
        (DESTINATION / "fabrication-approval.json").write_text(
            json.dumps({
                ...
                "external_gates": {
                    "mating_carrier": "B2.2",
                    "live_part_inventory_reviewed": False,
    ...
    print(f"R2.6 JLCPCB candidate built at {DESTINATION}")
