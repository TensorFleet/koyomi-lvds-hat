#!/usr/bin/env python3
"""Build and fail-closed audit the compact LCD R2.8 JLCPCB candidate."""
...
SOURCE = ROOT / "variants/koyomi-lvds-hat-rgb-compact-r2.8-c91592"
...
DESTINATION = ROOT / "fab/r2.8-compact-rgb-production-jlc-c91592"
PREFIX = "koyomi-lvds-rgb-compact-r2.8-jlc"
EXPECTED_REFS = {
...
def load_positions(path):
    ...
    if {row["Side"] for row in rows} != {"top"}:
        raise RuntimeError("R2.8 must remain single-side assembly")
    return rows
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".lcd-r28-jlc-", dir=ROOT) as temp:
        work = Path(temp)
        ...
        if "[excluded]" in errors_drc.read_text(encoding="utf-8"):
            raise RuntimeError("R2.8 contains an error-level DRC exclusion")
        warning_types = Counter(re.findall(
            r"^\[([^]]+)\]", all_drc.read_text(encoding="utf-8"), re.MULTILINE
        ))
        if warning_types != EXPECTED_DRC:
            raise RuntimeError(f"R2.8 all-severity baseline changed: {warning_types}")
        ...
        if '"result": "PASS"' not in audit:
            raise RuntimeError("R2.8 routed-interface audit failed")
        ...
        if '"result": "PASS"' not in edge_audit:
            raise RuntimeError("R2.8 edge-connector placement audit failed")
    ...
    print(f"R2.8 JLCPCB candidate built at {DESTINATION}")
