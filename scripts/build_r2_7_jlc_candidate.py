#!/usr/bin/env python3
"""Build and fail-closed audit the compact LCD R2.7 JLCPCB candidate."""
...
SOURCE = ROOT / "variants/koyomi-lvds-hat-rgb-compact-r2.7-30pct-floorplan"
...
DESTINATION = ROOT / "fab/r2.7-compact-rgb-production-jlc-candidate"
PREFIX = "koyomi-lvds-rgb-compact-r2.7-jlc"
EXPECTED_REFS = {
...
PARTS = {
    ...
    "FL1": ("DLP2ADN121HL4L", "C710576"),
    "FL2": ("DLP2ADN121HL4L", "C710576"),
...
EXPECTED_DRC = Counter({"lib_footprint_mismatch": 13})
...
def load_positions(path):
    ...
    if {row["Side"] for row in rows} != {"top"}:
        raise RuntimeError("R2.7 must remain single-side assembly")
    return rows
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".lcd-r27-jlc-", dir=ROOT) as temp:
        work = Path(temp)
        ...
        run(KICAD_CLI, "pcb", "export", "gerbers", "--layers", layers,
            "-o", gerbers, BOARD)
        ...
        run(KICAD_CLI, "pcb", "drc", "--severity-error", "--severity-exclusions",
            "--exit-code-violations", "--format", "report", "-o", errors_drc, BOARD)
        run(KICAD_CLI, "pcb", "drc", "--severity-all", "--severity-exclusions",
            "--format", "report", "-o", all_drc, BOARD)
        run(KICAD_CLI, "sch", "erc", "--severity-error", "--exit-code-violations",
        ...
        if "[excluded]" in errors_drc.read_text(encoding="utf-8"):
            raise RuntimeError("R2.7 contains an error-level DRC exclusion")
        warning_types = Counter(re.findall(
            r"^\[([^]]+)\]", all_drc.read_text(encoding="utf-8"), re.MULTILINE
        ))
        if warning_types != EXPECTED_DRC:
            raise RuntimeError(f"R2.7 all-severity baseline changed: {warning_types}")
        ...
        if '"result": "PASS"' not in audit:
            raise RuntimeError("R2.7 routed-interface audit failed")
        ...
        if '"result": "PASS"' not in edge_audit:
            raise RuntimeError("R2.7 edge-connector placement audit failed")
        ...
        (reports / "assembly-audit.txt").write_text(
            ...
            "FL1-FL2 C710576 stock 0/consign min 23 (checked 2026-08-20)\n"
        ...
        (reports / "parts-sourcing.txt").write_text(
            ...
            "FL1,FL2 C710576 / Murata DLP2ADN121HL4L: stock 0; "
            "consignment minimum 23\n"
            "Do not substitute without a new footprint, pin-order, mating, "
        ...
        (DESTINATION / "README.md").write_text(
            ...
            "eleven-net routed-interface audit passes. The 13 all-severity DRC "
            "warnings are inherited library-copy mismatches and are retained in "
            "the report.\n\n"
            ...
            "442-piece pre-order minimum and C710576 at stock 0 with a "
            "23-piece consignment minimum. The PCB is ready to fabricate; "
            "PCBA must wait until those exact parts are received.\n",
            encoding="utf-8",
        )
            ...
            json.dumps({
                ...
                "external_gates": {
                    ...
                    "zero_stock_parts": {
                        ...
                        "C710576": {"method": "consignment", "minimum": 23},
                    },
    ...
    print(f"R2.7 JLCPCB candidate built at {DESTINATION}")
